...
class QueryRequest(BaseModel):
    ...
    top_k: int = Field(
        default=5, ge=1, le=20, description="Final number of chunks to use for the answer"
    )
...
class IngestRequest(BaseModel):
    ...
    rebuild: bool = Field(
        default=False, description="Drop and recreate the collection before ingesting"
    )
